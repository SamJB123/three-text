...
if (!Number.isFinite(runs) || runs <= 0) {
  console.error('Usage: node bench/benchTessellator.mjs --runs 5');
  process.exit(1);
}

const targetBenches = [
  'process ttf removeOverlaps:false square',
  'process ttf removeOverlaps:false square+hole',
  'process ttf removeOverlaps:false multi(5)',
  'process ttf removeOverlaps:true square+hole',
  'process cff removeOverlaps:false square+hole'
];
...
for (let i = 1; i <= runs; i++) {
  const out = execFileSync(
    ...
    ['vitest', 'bench', 'bench/tessellator.bench.ts', '--run'],
    { encoding: 'utf8' }
  );
