...
if (!Number.isFinite(runs) || runs <= 0) {
  console.error('Usage: node bench/benchExtruder.mjs --runs 5');
  process.exit(1);
}

const targetBenches = [
  'extrude depth:0 square+hole',
  'extrude depth:7 square+hole',
  'extrude depth:0 ring(128)',
  'extrude depth:7 ring(128)'
];
...
for (let i = 1; i <= runs; i++) {
  const out = execFileSync(
    ...
    ['vitest', 'bench', 'bench/extruder.bench.ts', '--run'],
    { encoding: 'utf8' }
  );
