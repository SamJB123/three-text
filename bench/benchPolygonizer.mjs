...
if (!Number.isFinite(runs) || runs <= 0) {
  console.error('Usage: node bench/benchPolygonizer.mjs --runs 5');
  process.exit(1);
}

const targetBenches = [
  'polygonizeQuadratic - batch(100)',
  'polygonizeCubic - batch(100)',
  'mixed curves - typical glyph(20)'
];
...
for (let i = 1; i <= runs; i++) {
  const out = execFileSync(
    ...
    ['vitest', 'bench', 'bench/polygonizer.bench.ts', '--run'],
    { encoding: 'utf8' }
  );
